...
double eval (Expr* expression, double xValue, int* error, char* message) {
    ...
    switch (expression->tag) {
        ...
        case VAR_EXPR: 
            ...
            break;
        case BINOP_EXPR:
            l = eval (expression->val2.left, xValue, error, message);
            r = eval (expression->val3.right, xValue, error, message);

            if (isnan (l) || isnan (r)) *error = 2;
            if (expression->val1.binop == mdiv && r == 0) *error = 3;
            else if (expression->val1.binop == pow && l == 0 && r == 0) *error = 4;

            d = expression->val1.binop (l, r);
