    ...
    if (c == 'x') { //Variable
        ...
        if (!found) {
            ...
            if (!found) {
                ...
                *error = 1;
                ...
                ex->val2.message = malloc((strlen(function) + 1) * sizeof(char));
        ...
        if (!constant && (c = read_char(inp)) != ')') {
            ...
            *error = 3;
            ...
            ex->val2.message = malloc(2 * sizeof(char));
...
static Expr* expression(char** inp, int* error, char* message) { //Expression consist of ingredients and signs '+' and '-'
    ...
    if (c == '-') { //if it was negative ingredient multiply by -1 
        ...
        // -x <=> (-1) * x
        if ((ex1 = malloc(sizeof(Expr))) == NULL || (res = malloc(sizeof(Expr))) == NULL)
            return runtime_error(error, message);
    ...
    if ((c = read_char(inp)) == NUMBER) {
        ...
        if ((c = read_char(inp)) == ')') return res;
            ...
            *error = 3;
            ...
            res->tag = ERROR_EXPR;
            res->val1.error = 3;
            res->val2.message = malloc (sizeof(char) * 2);
        ...
        res->tag = ERROR_EXPR;
        res->val1.error = 2;
        res->val2.message = malloc(sizeof (char) * 5);
